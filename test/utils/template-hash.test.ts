...
describe("loadHashes / saveHashes", () => {
  ...
  it("loadHashes returns empty object for invalid JSON", () => {
    const hashesPath = path.join(tmpDir, ".aim-studio", ".template-hashes.json");
...
describe("initializeHashes", () => {
  ...
  it("hashes files in managed directories", () => {
    ...
    fs.mkdirSync(path.join(tmpDir, ".aim-studio", "scripts"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".aim-studio", "scripts", "task.py"), "print('hello')");
    ...
    const hashes = loadHashes(tmpDir);
    expect(hashes).toHaveProperty(".aim-studio/scripts/task.py");
    expect(hashes).toHaveProperty(".claude/commands/start.md");
  });

  it("excludes workspace and tasks directories", () => {
    fs.mkdirSync(path.join(tmpDir, ".aim-studio", "workspace"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".aim-studio", "workspace", "data.md"), "user data");
    fs.mkdirSync(path.join(tmpDir, ".aim-studio", "tasks"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".aim-studio", "tasks", "task.json"), "{}");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // These should be excluded
    expect(hashes).not.toHaveProperty(".aim-studio/workspace/data.md");
    expect(hashes).not.toHaveProperty(".aim-studio/tasks/task.json");
    expect(count).toBe(0);
  });
});
