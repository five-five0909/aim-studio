...
import {
  ...
  commonCliAdapter,
  getAllScripts,
} from "../src/templates/aim/index.js";
...
describe("regression: Windows path separator (beta.12)", () => {
  it("[beta.12] isManagedPath handles Windows backslash paths", () => {
    ...
    expect(isManagedPath(".aim-studio\\spec\\backend")).toBe(true);
  });
...
describe("regression: task.py _resolve_task_dir path handling", () => {
  it("[beta.12] task.py resolve_task_dir handles .aim-studio prefix", () => {
    // The function should recognize .aim-studio-prefixed paths as relative paths
    expect(taskScript).toContain('.startswith(".aim-studio")');
  });
